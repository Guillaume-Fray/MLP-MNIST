...
for x=1:10000
    ...
    m = m.adapt_to_target([0], [0], 0.8); % rate = 0.1
    m = m.adapt_to_target([1], [1], 0.8); % rate = 0.1
    m = m.adapt_to_target([2], [0], 0.8); % rate = 0.1
    % Outputs should approach [0 1 0]
    [m.compute_output([0]) m.compute_output([1]) m.compute_output([2])];
end

disp('----- Outputs -----');
disp([m.compute_output([0]) m.compute_output([1]) m.compute_output([2])]);     
